...
class response:
	...
	def __init__(self,args):
		...
		self.max_length=args.n
		self.beam_width=args.beam_width
		self.relevance=args.relevance
		self.temperature=args.temperature
		self.topn=args.topn
		self.states = self.initial_state_with_relevance_masking(self.net, self.sess, self.relevance)
		print("INITIALIZED\n")
	...
	def chat(self,user_input):
		...
		if not user_command_entered:
			self.states = self.forward_text(self.net, self.sess, self.states, self.relevance, self.vocab, self.sanitize_text(self.vocab, "> " + user_input + "\n>"))
			computer_response_generator = self.beam_search_generator(sess=self.sess, net=self.net,
				initial_state=copy.deepcopy(self.states), initial_sample=self.vocab[' '],
				early_term_token=self.vocab['\n'], beam_width=self.beam_width, forward_model_fn=self.forward_with_mask,
